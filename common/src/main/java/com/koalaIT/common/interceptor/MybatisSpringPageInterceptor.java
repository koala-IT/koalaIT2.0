...
public class MybatisSpringPageInterceptor implements Interceptor {
	...
	protected ThreadLocal<QueryPage> pageThreadLocal = new ThreadLocal<QueryPage>();
	...
	public Object intercept(Invocation invocation) throws Throwable {
		if (invocation.getTarget() instanceof StatementHandler) { // 控制SQL和查询总数的地方
			QueryPage page = pageThreadLocal.get();
			if (page == null) { //不是分页查询
			...
			// 获取是否有分页Page对象
			QueryPage page = findPageObject(invocation.getArgs()[1]);
			if (page == null) {
				if (log.isTraceEnabled()) {
	...
	protected QueryPage findPageObject(Object parameterObj) {
		if (parameterObj instanceof QueryPage) {
			return (QueryPage) parameterObj;
		} else if (parameterObj instanceof Map) {
			for (Object val : ((Map<?, ?>) parameterObj).values()) {
				if (val instanceof QueryPage) {
					return (QueryPage) val;
				}
			}
		}
		return null;
	 ...
	 * @return
	 */
	protected String buildPageSql(QueryPage page, String sql) {
	 ...
	 * @return
	 */
	protected String buildMysqlPageSql(QueryPage page, String sql) {
	 ...
	 * @return
	 */
	protected String buildOraclePageSql(QueryPage page, String sql) {
	 ...
	 * @throws SQLException
	 */
	protected void queryTotalRecord(QueryPage page, Object parameterObject, MappedStatement mappedStatement, Connection connection) throws SQLException {
